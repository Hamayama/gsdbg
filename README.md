...
- スクリプトファイル内で (use gsdbg) によりモジュールを読み込み、  
  ...
  - `(gsdbg [:pa prompt-add] [:lv local-vars] [:rv ret-val])`  
    ...
    - キーワード引数 :lv の local-vars には、ローカル変数の名前を、  
      ...
      また、後述の getlv / setlv 手続きで参照と更新が可能です。  
...
- デバッガ内では、以下のコマンドが使用可能です。  
  ...
  - `,quit [code]`  
    終了コードを code としてスクリプトを終了します (本当に終了するか、確認を求められます)。  
...
- 2020-3-20  v2.02 ,quitコマンドの終了確認処理を追加
- 2020-3-20  v2.03 setlvのvalue引数が2回評価されていた件の修正等
- 2020-3-20  v2.04 ,continueと,goコマンドは、デバッガ実行中のみ処理するように変更
- 2020-3-22  v2.05 エラーチェック追加。一部処理見直し等
